"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { supabase } from "@/lib/supabase";
import type { Profile, StudentProfile } from "@/types";

export default function StudentDashboard() {
  const router = useRouter();
  const [profile, setProfile] = useState<Profile | null>(null);
  const [studentProfile, setStudentProfile] = useState<StudentProfile | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const load = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        router.push("/auth/login");
        return;
      }

      const { data: profileData } = await supabase
        .from("profiles")
        .select("*")
        .eq("user_id", user.id)
        .single();

      if (!profileData) {
        // 프로필 미생성 계정 → 재가입 유도
        router.push("/auth/signup");
        return;
      }
      if (profileData.user_type !== "student") {
        // 잘못된 대시보드 접근 → 올바른 대시보드로
        router.push("/dashboard/owner");
        return;
      }
      setProfile(profileData);

      const { data: spData } = await supabase
        .from("student_profiles")
        .select("*")
        .eq("user_id", user.id)
        .single();
      setStudentProfile(spData);
      setLoading(false);
    };
    load();
  }, [router]);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-500">로딩 중...</div>
      </div>
    );
  }

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-black text-gray-900">
          안녕하세요, {profile?.name}님
        </h1>
        <p className="text-gray-500 mt-1">대학생 대시보드</p>
      </div>

      {/* 프로필 등록 유도 */}
      {!studentProfile ? (
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-2xl p-8 mb-6">
          <h2 className="text-xl font-bold mb-2">프로필을 등록해보세요!</h2>
          <p className="text-orange-100 mb-4 text-sm">
            관심 분야와 경험을 등록하면 소상공인과 매칭될 수 있어요.
          </p>
          <Link
            href="/student-profile/register"
            className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-xl transition-colors"
          >
            프로필 등록하기 →
          </Link>
        </div>
      ) : (
        <div className="card mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{profile?.name}의 프로필</h2>
              <div className="flex flex-wrap gap-2 mt-2">
                {studentProfile.interests?.map((interest) => (
                  <span key={interest} className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full font-medium">
                    {interest}
                  </span>
                ))}
              </div>
            </div>
            <Link href="/student-profile/register" className="text-sm text-blue-600 hover:underline">
              수정
            </Link>
          </div>
          {studentProfile.portfolio_url && (
            <a
              href={studentProfile.portfolio_url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-sm text-blue-600 hover:underline flex items-center gap-1"
            >
              포트폴리오 보기
            </a>
          )}
        </div>
      )}

      {/* 빠른 메뉴 */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {[
          { href: "/student-profile/register", label: "프로필 관리", desc: "정보 수정" },
          { href: "/explore/shops", label: "매장 탐색", desc: "매장 찾기" },
          { href: "/matches", label: "매칭 요청", desc: "요청 관리" },
          { href: "#", label: "프로젝트", desc: "준비 중" },
          { href: "#", label: "리뷰", desc: "준비 중" },
          { href: `/profile/${profile?.user_id}`, label: "내 프로필", desc: "공개 페이지" },
        ].map((item) => (
          <Link
            key={item.label}
            href={item.href}
            className="card hover:shadow-md transition-shadow text-center"
          >
            <div className="font-semibold text-sm text-gray-900">{item.label}</div>
            <div className="text-xs text-gray-400 mt-0.5">{item.desc}</div>
          </Link>
        ))}
      </div>

    </div>
  );
}
